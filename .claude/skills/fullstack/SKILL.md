...
name: fullstack
description: >
  Modern full-stack web development — React/Next.js, Node APIs, databases, auth, deployment. Use when
  building web apps, REST/GraphQL APIs, SSR/SSG, Prisma/Postgres, or the user mentions Next.js, Express,
  or frontend and backend together.
license: MIT
---

# Full-Stack Developer

Expert full-stack development for modern JavaScript/TypeScript stacks.

## Project defaults

- **No hard-coded user-facing strings** — route UI copy through i18n from the start; retrofitting is expensive
- **Tests for logic** — APIs and business rules get tests; write them alongside the code, not after
- **New repos** — a root `Makefile` with `dev`, `up`, `down` so the project starts the same way every time
- **Scope** — finish one slice to a working, verified state before starting the next
